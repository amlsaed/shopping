...
function drawUI(getproducts = []){
    ...
    if(JSON.parse(localStorage.getItem("productsfavorite")).length ===0) {noproducts.innerHTML=`
       <span>No products in the shopping card</spn>
    `}
    let products = JSON.parse(localStorage.getItem("productsfavorite")) || getproducts;
                    ...
                    <div class="product-details">
                      ...
                      <div class="product-bottom-details">
                        ...
                        <div class="product-links">
                          <button onclick="removeFromCards(${item.id})">Rermove from favorite</button>
                        </div>
                      </div>
                    </div>
                  </div>
...
function removeFromCards(id){
   if(JSON.parse(localStorage.getItem("productsfavorite"))){
    let items = JSON.parse(localStorage.getItem("productsfavorite"))
    let filtered = items.filter((item)=>item.id !== id)
    localStorage.setItem("productsfavorite",JSON.stringify(filtered))
    drawUI(filtered)
